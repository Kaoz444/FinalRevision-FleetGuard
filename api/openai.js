export default async function handler(req, res) {
    ...
    try {
        ...
        // 🛠️ Validar que OpenAI realmente devolvió JSON válido y estructurado
        ...
        let parsedResponse;
        try {
            // **IMPORTANTE**: OpenAI ya devuelve JSON estructurado, NO hay que volver a parsearlo.
        ...
        // ✅ Si la IA respondió correctamente, devolver la respuesta directamente
...
/*export default async function handler(req, res) {
    ...
    try {
        ...
        // Verificar si la IA respondió con contenido válido
        const rawResponse = data.choices[0]?.message?.content;
        if (!rawResponse) {
            console.error("❌ Respuesta vacía o inválida de OpenAI:", JSON.stringify(data, null, 2));
            return res.status(500).json({ error: "Formato incorrecto en la respuesta de OpenAI" });
        }
        ...
        if (typeof rawResponse === "string") {
            ...
            console.error("❌ Respuesta en formato inesperado de OpenAI:", rawResponse);
            return res.status(500).json({ error: "Formato inesperado en la respuesta de OpenAI" });
        }
